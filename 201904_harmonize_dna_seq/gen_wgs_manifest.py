...
CUSTOM_BAM_ROOT = 'processed_data/wgs_mark_dup_bam'

# Path to the output manifest file
PREV_OUT_MANIFEST_PTH = '../201904_locate_adhoc_data/tracked_results/MGI.GBM_custom_wgs.BamMap.dat'
OUT_MANIFEST_PTH = '../201907_locate_data/tracked_results/MGI.GBM_custom_wgs.BamMap.dat'
...
all_wgs_custom_bams = sorted(Path(CUSTOM_BAM_ROOT).resolve().glob('*.bam'))
wgs_bam_map = {}
for pth in all_wgs_custom_bams:
    case, sample_type = pth.stem.split('_', 1)
    wgs_bam_map[(case, sample_type)] = pth
...
with open(OUT_MANIFEST_PTH, 'w') as f:
    ...
    for (case, sample_type), data_pth in wgs_bam_map.items():
        ...
        sample_name = f'{case}.WGS.{abbrv}.hg38-bobo'
        ...
        writer.writerow([
            sample_name, case, 'GBM', 'WGS', sample_type,
