"""Extract TCGA MC3 mutation calls GBM only."""
import argparse
import csv
import gzip
import logging
...
def get_all_tcga_gbm_cases(tsv_pth):
...
def get_all_tcga_gbm_tss_codes(tsv_pth):
    # TSS = Tissue Source Site
    with gzip.open(tsv_pth, 'rt') as f:
        reader = csv.DictReader(f, dialect='excel-tab')
        all_gbm_tss_codes = set(
            row['TSS Code'] for row in reader
            if row['Study Name'] == 'Glioblastoma multiforme'
        )
    return all_gbm_tss_codes


def main(maf_pth, case_pth, tss_pth):
    logger.info(
        f'Read MC3 MAF from {maf_pth}, all GBM cases from {case_pth}, TCGA TSS code from {tss_pth}'
    )
    maf = MC3MAF(maf_pth)
    all_tcga_gbm_cases = get_all_tcga_gbm_cases(case_pth)
    all_tcga_gbm_tss_codes = get_all_tcga_gbm_tss_codes(tss_pth)
    ...
    for i, mut in enumerate(maf, 1):
        ...
        is_gbm_tss = tss in all_tcga_gbm_tss_codes
        is_gbm_case = case_id in all_tcga_gbm_cases
        if is_gbm_tss:
            if not is_gbm_case:
                logger.error(f'{i}-th mutation has GBM TSS {tss_id} but not a GBM case {case_id}')
            seen_gbm_cases.add(case_id)
            # Export the mutation
            print('\t'.join(mut))
        elif is_gbm_case:
            logger.error(f'{i}-th mutation has no GBM TSS {tss_id} but is a GBM case {case_id}')

    logger.info(f'All mutations are processed. '
                f'Detected {len(seen_gbm_cases)} GBM cases with MC3 mutations.')
...
def setup_cli():
    ...
    parser.add_argument('case_tsv', help="Path to all TCGA GBM cases")
